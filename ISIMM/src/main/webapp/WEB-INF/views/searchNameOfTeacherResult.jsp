...
<body>
	...
	<section id="main-content">
		<section class="wrapper site-min-height">
			...
			<div class="row mt">
				<div class="col-lg-12">
					<a href="${pageContext.request.contextPath}/searchTeacher"><button
							class="btn btn-primary">Search Teacher</button></a><br> <br>
					<h1>Search Result</h1>
					<c:set value="${teacher}" var="teacherList" />
					<table class="table table-hover table-bordered">
						...
						<c:forEach items="${teacherList.pageList}" var="teacher"
							varStatus="status">
							<tr>
								...
								<td><a class="mr-3"
									href="${pageContext.request.contextPath}/viewTeacher?userId=${teacher.userId }"><input
					...
					<div class='d-flex'>
						<ul class="pagination mx-auto">
							<c:choose>
								...
								<c:when test="${teacherList.firstPage}">
									<li><span class="page-link">Prev</span></li>
								</c:when>
								<c:otherwise>
									<c:url value="prev" var="url" />
									<li><a class="page-link"
										href='<c:out value="${pageContext.request.contextPath}/searchNameOfTeacher/${url}?searchNameOfTeacher=${search}&submit=Search" />'>Prev</a></li>
								</c:otherwise>
							</c:choose>
							<c:forEach begin="1" end="${teacherList.pageCount}" step="1"
								varStatus="tagStatus">
								<c:choose>
									<%-- In PagedListHolder page count starts from 0 --%>
									<c:when test="${(teacherList.page + 1) == tagStatus.index}">
										<li><span class="page-link">${tagStatus.index}</span></li>
									</c:when>
									<c:otherwise>
										<c:url value="${tagStatus.index}" var="url" />
										<li><a class="page-link"
											href='<c:out value="${pageContext.request.contextPath}/searchNameOfTeacher/${url}?searchNameOfTeacher=${search}&submit=Search" />'>${tagStatus.index}</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<c:choose>
								<%-- Show Next as link if not on last page --%>
								<c:when test="${teacherList.lastPage}">
									<li><span class="page-link">Next</span></li>
								</c:when>
								<c:otherwise>
									<c:url value="next" var="url" />
									<li><a class="page-link"
										href='<c:out value="${pageContext.request.contextPath}/searchNameOfTeacher/${url}?searchNameOfTeacher=${search}&submit=Search" />'>Next</a></li>
								</c:otherwise>
							</c:choose>
						</ul>
					</div>
