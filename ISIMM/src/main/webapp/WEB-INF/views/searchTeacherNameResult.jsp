		...
		<section class="wrapper site-min-height">
			...
			<div class="row mt">
				<div class="col-lg-12">
					...
					<a href="${pageContext.request.contextPath}/createClassroomForm"><button
					...
					<table class="table table-hover table-bordered">
						...
						<c:forEach items="${classroomList.pageList}" var="classroom" varStatus="status">
							<tr>
							...
							<!-- The Modal -->
							<div class="modal fade" id="deleteModal${classroom.classroomId }">
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body" align="left">Are you sure you
											want to delete the classroom?</div>

										<!-- Modal footer -->
										<div class="modal-footer">
											<a href="${pageContext.request.contextPath}/deleteClassroom?classroomId=${classroom.classroomId }"><button
					...
					<div class='d-flex'>
						<ul class="pagination mx-auto">
							<c:choose>
								...
								<c:when test="${classroomList.firstPage}">
									<li><span class="page-link">Prev</span></li>
								</c:when>
								<c:otherwise>
									<c:url value="prev" var="url" />
									<li><a class="page-link"
										href='<c:out value="${pageContext.request.contextPath}/searchTeacherName/${url}?searchTeacherName=${search}&submit=Search" />'>Prev</a></li>
								</c:otherwise>
							</c:choose>
							<c:forEach begin="1" end="${classroomList.pageCount}" step="1"
								varStatus="tagStatus">
								<c:choose>
									<%-- In PagedListHolder page count starts from 0 --%>
									<c:when test="${(classroomList.page + 1) == tagStatus.index}">
										<li><span class="page-link">${tagStatus.index}</span></li>
									</c:when>
									<c:otherwise>
										<c:url value="${tagStatus.index}" var="url" />
										<li><a class="page-link"
											href='<c:out value="${pageContext.request.contextPath}/searchTeacherName/${url}?searchTeacherName=${search}&submit=Search" />'>${tagStatus.index}</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<c:choose>
								<%-- Show Next as link if not on last page --%>
								<c:when test="${classroomList.lastPage}">
									<li><span class="page-link">Next</span></li>
								</c:when>
								<c:otherwise>
									<c:url value="next" var="url" />
									<li><a class="page-link"
										href='<c:out value="${pageContext.request.contextPath}/searchTeacherName/${url}?searchTeacherName=${search}&submit=Search" />'>Next</a></li>
								</c:otherwise>
							</c:choose>
						</ul>
					</div>
